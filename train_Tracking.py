...
def TrainTracking():
    ...
    if Player.GetSkillValue("Tracking") == Player.GetSkillCap("Tracking"):
        Player.HeadMessage(
            "Your Tracking is already at its skill cap!", colors["green"]
        )
        return
    ...
    while not Player.IsGhost and Player.GetSkillValue("Tracking") < Player.GetSkillCap(
        "Tracking"
    ):
    ...
    Player.HeadMessage(
        "Congratulations! Your Tracking is now at its skill cap!", colors["green"]
    )
